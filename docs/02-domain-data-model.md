...
struct MemberDisk {                // 唯一业务对象，由 MemberDisk Domain 独占修改
    ...
    shrink_requested: bool,        // 尚未完成的计划移除意图
    io_state: Up | Down,           // Pool 已提交的有效 IO 能力
...
`MemberDisk` 本身就是核心对象，不能用状态机枚举代替，也不再拆成 `Spec + Record + Snapshot`。DiskMap 的 DOWN/UP 只作为 `MemberDiskEvent` 输入，由执行槽持有，不在逻辑对象中复制或持久化；`observed_at` 同样属于该事件。Shrink 是 Pool 自己接受且必须跨主恢复的管理意图，因此当前保存在 `shrink_requested`。对象中的 `io_state` 表示 Pool 已经完成对应 user_dp 动作后的有效 IO 能力；UP 只有在 `OpenDisk -> SetDiskState::Up` 两阶段成功后提交。执行槽和 Future 状态不进入业务对象。
...
该枚举只是 `project(io_state, allocation_state, membership_state)` 的结果，只表达合法组合。它不是新的持久化实体，也不包含容量、Tier、故障域和位图。`DownActive` 中的 `Active` 只表示分配策略尚未关闭，不表示系统可以向 IO 能力为 Down 的盘实际分配：

\[
Allocatable(m)=IoUp(m)\land AllocationActive(m)\land Member(m)
\]

`DI` 和 `UI` 都表示禁止新分配；区别是当前已经提交的 IO 能力。`DA -> DI` 本身不是不可逆承诺，排空可以协作停止并通过 UP 事件回到 `UA`。已提交的单个 BG remap 不要求回滚。`REMOVED` 是保留身份和历史元数据的稳定状态，不删除 MemberDisk 对象；之后收到 UP 时执行 Rejoin，再回到 `UA`。
...
该结构属于 Tier/MemberDisk 的空间分配数据，不改变 Pool 的顶层领域边界。当前 Rust 纵切面使用语义等价的 `AllocationBitmap` 验证 BLK 分配/释放和 1GiB/2GiB 粒度；它不是最终 SDB Partition 物理布局。磁盘槽位稳定性、Partition 跨 Tier 关系以及并行更新机制均为后续专项设计。
