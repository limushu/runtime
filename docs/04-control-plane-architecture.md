...
`Pool` 不叫 `PoolRuntime`：前者是业务概念，后者只是未来可能提炼的执行机制。当前只有 MemberDisk 纵切面使用一个根 task poll 多盘 Future；没有通用 Runtime 或虚拟 ActorCell。
...
当前不规定所有领域必须服从一个 Service Runtime。MemberDisk 先保留一条具体且完整的实现链：私有 command、typed oneshot、单根 Future poll、盘级 active reconciliation/waiters 和协作取消。业务方法仍是自然 `async fn`；`MemberDiskService::run` 直接拥有事件队列、Future 集合和对象槽这些局部运行态，不再引入第二个 `ServiceLoop` 类型。

两种实现都必须使用指向明确目标实例的 Handle/facade。请求类型本身不声明目的地，也不存在依据请求枚举自动选择服务的全局 Router。是否统一生命周期、观测和对象槽位，等待第二个完整领域实现后再决定。
...
命令枚举、channel 和 oneshot 对领域外部不可见。MemberDisk 的每个私有命令直接携带自身 typed oneshot，不需要统一 Response enum。业务代码不通过 Task 发起跨服务通信。
...
- 一个 MemberDisk Service 实例只有一个执行 `MemberDiskService::run` 的根 task，不为每盘创建 Tokio task/mailbox。
- 盘级 `operation_cancel` 只中止恢复等待、排空和上线等可替换步骤；`set_disk_down` 不接收任何服务级 token，并在 MemberDisk 领域内重试通用广播。关闭全部 Client 只进入 Drain 并等待已有 Future 收敛；强制停止由持有者对根 `JoinHandle` 执行 abort。

因此当前逻辑上的 MemberDisk 控制单元是 `MemberDisk`、`MemberDiskService::reconcile_once` 与同一 `DiskUuid` 下的私有执行槽。对象保存已提交能力和持久化意图，状态表决定下一步，执行槽保存 active/pending 事件及其 Future 控制信息。
...
- 独立控制通道、Task 与 Trace 属于后续运行时能力，当前纵切面尚未实现；
- 跨服务通信属于显式 Service facade/Client，不属于 Task；
- 核心元数据只能由所属 Service 修改，且不得把可变访问跨过 `.await`。

已经由当前 MemberDisk 纵切面验证：

- 当前 MemberDisk 使用一个领域专属根 task poll 事件、查询和多盘 reconciliation Future；
- MemberDisk 对象、Service 状态表/业务步骤与 Runtime 私有执行槽的职责分离；
- 自然 `async fn` 业务方法、显式领域 facade、完整收敛路径，以及新事件等待旧 Future 稳定退出后重新计算。

尚未实现的是 `PoolManager -> Pool -> Domain Client` 多 Pool 装配、独立控制通道、Task/Trace 观测和通用 Runtime。仍待验证的是 Tier/Partition 并行化、完整 VD/BG 恢复语义和真实 SDB 条件写适配器，而不是重新引入隐藏路由或每对象 task。
