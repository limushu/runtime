...
查询不创建 Task，但仍作为普通 handler Future 由根循环执行。需要互斥的盘事件必须通过目标 `MemberDiskClient::call(event)` 进入，不能绕过入口直接并发调用写流程。当前已把 active/pending/cancel/promotion/waiter 下沉为不含 Disk 业务词汇的 `ObjectTaskCoordinator<K, I, E>`；Key、冲突判断和下一步业务动作仍由领域实现，不能继续下沉为通用状态机或 Action DSL。

## 共享意图的生命期

多个提交合并到同一对象意图时，Workflow 的生命期属于对象意图槽位，不属于第一个提交者。当前 `call(MemberDiskEvent)` 返回 `Accepted`，只确认输入已经被 Service 校验并送入对象槽，不订阅最终业务结果；需要等待的测试或管理请求使用 `call(WaitMemberDiskIdle(...))`：

- `MemberDiskClient` 被 clone 或 drop 不改变已接收 Workflow 的生命期；
- `WaitMemberDiskIdle` 的接收端离开不取消 Workflow；
