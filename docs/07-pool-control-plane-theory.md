...
MemberDisk
  = persisted decision fields
    (identity + pool/tier + media/capacity + failure domains
     + IO capability + allocation state + membership
     + BLK bitmap + revision)
PhysicalState = DiskMap event input
...
enum MemberDiskState {
    ...
    DownActive,     // DA：IO能力Down；分配策略尚未关闭
    DownInactive,   // DI：IO能力Down，禁止新分配
    UpInactive,     // UI：IO能力Up，禁止新分配
    Removed,        // 已解除成员关系
}
```
...
例如，硬盘离线后的最终目标是把 MemberDisk 安全推进到下一个稳态，因此 MemberDisk 状态表及其业务步骤属于 DiskDomain。DiskDomain 可以调用：
...
- 需要对象级互斥、重复合并、冲突替换或顺序排队；
- 需要独立 TaskAttempt 进度、取消、Trace 或资源计量。

因此 Query、只读 Snapshot 和立即完成的状态判断不需要可观测 Task。它们仍然是由 Future Driver poll 的 Request Future。Task 不是请求进入系统后的强制包装，更不是业务开发者编写 `async fn` 的语法前提。
...
当领域决定停止时，Runtime 通过共享的取消作用域唤醒整个调用链；父 Task 不直接 drop 子 Future。显式 Service Client 等待下游停止、完成当前不可中断动作或到达安全提交点并返回稳定结果。当前业务 step 在恢复窗口和下游调用边界响应取消；运行循环先接纳下游稳定结果，再根据最新对象决定是否进入下一步。跨模块通信由命名的领域 facade 提供，Task 不是 RPC 能力所有者。
...
Shrink 期间收到 DOWN 不是旁路命令。`shrink_requested=true` 保存在对象中，DOWN 保存在 pending 事件中：在途在线排空先协作停止并稳定返回，DOWN 成为 active 后从 `(UI, DOWN event, true)` 执行 `set_disk_down`，再从 `(DI, DOWN event, true)` 继续排空。没有向运行中 Future 注入命令。
...
`REMOVED` 是保留 MemberDisk 身份的稳定状态，而不是对象删除。之后收到 UP 时执行 Rejoin，并在 `OpenDisk -> SetDiskState::Up` 成功后提交 `MemberDiskUpdate::Rejoin` 回到 `UA`。本例仍用于证明“对象状态拥有业务真相、执行过程协作取消、局部不可逆不等于整体不可取消”。
...
11. 对比第二个真实领域，从具体 runtime 中识别可提炼的最小公共机制；
12. 首阶段串行运行时及安全并行演进；
...
```text
对象状态表回答：基于完整对象，下一项业务步骤是什么
领域事实入口回答：权威对象发生了什么变化
Runtime执行槽回答：同一个Key如何保持单Future、取消和重新计算
Reconcile回答：现在离稳态还差什么，以及业务步骤如何调用其他领域能力
Operation Context回答：这次影响为何发生、属于谁、如何关联
Task Attempt回答：本次由什么执行实例驱动Future
Runtime回答：Future如何被并发驱动、强制清理和观测
```

因此，Scheduler、Actor 和 Task 不应成为业务模型的中心；它们是承载业务模型的执行基础。Pool 是隔离边界，领域对象是事实载体，可执行状态表是复杂对象的业务决策核心，直接业务步骤或简单线性 Future 是过程表达，Operation Context 是稳定因果身份，CausalGraph 是可解释性基础，Reconcile 是恢复保障。框架的价值不是统一所有业务，而是隐藏通信、对象串行化、结构化并发、取消传播和观测这些重复机制，使开发者把主要精力放在状态迁移、业务步骤与不变量上。
