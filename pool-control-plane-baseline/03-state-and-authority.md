...
| MemberDisk 身份映射、Pool/Tier、介质、容量、故障域 | SDB 中的 `MemberDiskRecord`；物理盘基础事实由 DiskMap 校验 | MemberDisk Actor 持有的领域实体 |
| MemberDisk 空间位图 | SDB | 分配决策的内存工作集 |
| MemberDisk 物理 UP/DOWN | DiskMap 当前观测 | Actor 中的瞬时事实，不覆盖 SDB 决策 |
| MemberDisk Allocation/Membership | SDB 中的 `MemberDiskRecord` | Actor 中的决策投影 |
| MemberDisk UA/DA/DI/UI/Removed | 由物理观测、Allocation、Membership 派生 | 只读运行投影，不是第二份核心元数据 |
| MemberDisk ObjectActivity | Service Runtime 的 ObjectSlot | 临时在途意图，不参与 SDB 业务真相 |
